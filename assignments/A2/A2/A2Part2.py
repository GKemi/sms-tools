...
corresponding to the frequency index k. In other words: calculate the complex sinusoid of a specific frequecy, 
as it would be found as a part of the DFT function. This is really just the e^-j*2*pi*k*n/N portion on it's own,
without an input signal x[n].

Note that the complex sinusoid used in DFT computation has a negative sign in the exponential function.
...
def genComplexSine(k, N):
    ...
    sines = np.array([])
    for i in range(N):
        sinusoid = np.exp(-1j * 2 * np.pi * k * i / N)

        sines = np.append(sines, sinusoid)
